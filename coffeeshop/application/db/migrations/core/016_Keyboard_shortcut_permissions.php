...
class Migration_Keyboard_shortcut_permissions extends Migration {
	...
	public function up() 
	{
		...
		$data = array(
			'name'        => 'Coffeeshop.UI.Manage' ,
			'description' => 'Manage the Coffeeshop UI settings' 
		);
	...
	public function down() 
	{
		...
		$query = $this->db->query("SELECT permission_id FROM {$prefix}permissions WHERE name = 'Coffeeshop.UI.Manage'");
		...
		//delete the role
		$this->db->query("DELETE FROM {$prefix}permissions WHERE (name = 'Coffeeshop.UI.Manage')");
