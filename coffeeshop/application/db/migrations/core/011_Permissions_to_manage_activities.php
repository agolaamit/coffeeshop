...
class Migration_Permissions_to_manage_activities extends Migration {
	...
	public function up() 
	{
		...
		$data = array(
		   array(
		      'name' 		=> 'Coffeeshop.Activities.Manage' ,
		...
		$permissions = $this->db->select('permission_id')->where("(name = 'Coffeeshop.Activities.Manage') OR (name LIKE 'Activities.%.View') OR (name LIKE 'Activities.%.Delete')")->get($prefix.'permissions')->result();
	...
	public function down() 
	{
		...
		if (isset($roles) && is_array($roles) && count($roles)) {
			foreach ($roles as $role) {
				...
				$query = $this->db->query("SELECT `permission_id` FROM `{$prefix}permissions` WHERE `name` LIKE 'Activities.%.View' or `name` LIKE 'Activities.%.Delete' or `name` = 'Coffeeshop.Activities.Manage'");
				...
				//delete the role
				$this->db->query("DELETE FROM `{$prefix}permissions` WHERE `name` LIKE 'Activities.%.View' or `name` LIKE 'Activities.%.Delete' or `name` = 'Coffeeshop.Activities.Manage'");
			}
		}
