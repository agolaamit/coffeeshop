...
class Migration_Version_02_upgrades extends Migration {
	...
	public function up() 
	{
		...
		$sql = "ALTER TABLE {$prefix}permissions
				ADD COLUMN `Coffeeshop.Emailer.View` TINYINT(1) DEFAULT 0 NOT NULL";
		$this->db->query($sql);	
		
		$this->db->query("UPDATE {$prefix}permissions SET `Coffeeshop.Emailer.View`=1 WHERE `role_id`=1");
	...
	public function down() 
	{
		...
		if ($this->db->field_exists('`Coffeeshop.Emailer.View`', 'permissions'))
		{
			$this->db->query("ALTER TABLE `{$prefix}permissions` DROP COLUMN `Coffeeshop.Emailer.View`");
		}
