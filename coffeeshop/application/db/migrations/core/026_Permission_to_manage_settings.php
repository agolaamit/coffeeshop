...
class Migration_Permission_to_manage_settings extends Migration {
	...
	public function up() 
	{
		...
		$this->db->query("INSERT INTO {$prefix}permissions(name, description) VALUES('Coffeeshop.Settings.View', 'To view the site settings page.')");
		...
		// add the permission
		$this->db->query("INSERT INTO {$prefix}permissions(name, description) VALUES('Coffeeshop.Settings.Manage', 'To manage the site settings.')");
		// give current role (or administrators if fresh install) full right to manage permissions
		$this->db->query("INSERT INTO {$prefix}role_permissions VALUES(1,".$this->db->insert_id().")");
	}
