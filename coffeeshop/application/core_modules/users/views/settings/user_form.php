...
<div class="admin-box">
    ...
    <fieldset>
        ...
        <div class="control-group <?php echo form_error('phone') ? 'error' : '' ?>">
            ...
            <div class="controls">
                <input type="text" name="phone" id="phone" value="<?php echo set_value('phone', isset($user) ? $user->phone : '') ?>">
                <?php if (form_error('phone')) echo '<span class="help-inline">' . form_error('phone') . '</span>'; ?>
            </div>
        </div>
        ...
        <div class="control-group <?php echo form_error('occupation') ? 'error' : '' ?>">
            ...
            <div class="controls">
                <input type="text" name="occupation" id="occupation" value="<?php echo set_value('occupation', isset($user) ? $user->occupation : '') ?>">
                <?php if (form_error('occupation')) echo '<span class="help-inline">' . form_error('occupation') . '</span>'; ?>
            </div>
        </div>
        ...
        <div class="control-group <?php echo form_error('dob') ? 'error' : '' ?>">
            ...
            <div class="controls">
                <input type="text" name="dob" id="dob" value="<?php echo set_value('dob', isset($user) ? $user->dob : '') ?>">&nbsp;(YY-MM-DD)
                <?php if (form_error('dob')) echo '<span class="help-inline">' . form_error('dob') . '</span>'; ?>
            </div>
        </div>
        ...
        <div class="control-group <?php echo form_error('cafe_id') ? 'error' : '' ?>">
            <label class="control-label" for="language">Cafe</label>
            <div class="controls">
                <select name="cafe_id" id="cafe_id" class="chzn-select">
                    <?php if (isset($cafes) && is_array($cafes) && count($cafes)) : ?>
                        <?php foreach ($cafes as $cafe) : 
                                // check if it should be the default
                                $default_cafe = FALSE;
                                if ((isset($user) && $user->cafe_id == $cafe->id) || (!isset($user)))
                                {
                                    $default_cafe = TRUE;
                                }
                            ?>
                
                            <option value="<?php echo $cafe->id ?>" <?php echo set_select('cafe_id', $cafe->id, $default_cafe) ?>>
                                <?php e(ucfirst($cafe->cafe_name)) ?>
                            </option>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </select>
                <?php if (form_error('cafe_id')) echo '<span class="help-inline">' . form_error('cafe_id') . '</span>'; ?>
            </div>
        </div>
...
<!--        <div class="control-group <?php echo form_error('language') ? 'error' : '' ?>">
    ...
    <?php if (isset($user) && has_permission('Coffeeshop.Roles.Manage') && has_permission('Permissions.' . $user->role_name . '.Manage') && isset($roles)) : ?>
        <fieldset>
