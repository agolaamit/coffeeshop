...
class Webservice extends REST_Controller {
        ...
        if ($json_array['body']['type'] == 'email') {
            ...
            if ($user) {
                ...
                if (do_hash($user->salt . $password) == $user->password_hash) {     
                    ...
                    $users_app_login = $this->users_app_login_model->select('*')->find_by('userid', $user->id);
    ...
    function editAccount() {
        ...
        if ($json_array['body']['userId'] != "") 
        {
            ...
            if($json_array['body']['sessionId'] == "")
            {
                echo json_encode(array('status' => 'error', 'message' => 'User session not available'));
                exit;
            }
            $this->load->model('users/users_app_login_model');
            $users_app_login = $this->users_app_login_model->select('*')->find_by('userid', $json_array['body']['userId']);            
            $allowed_time = time()-1800;
            if ($json_array['body']['sessionId'] == $users_app_login->session_id && $users_app_login->createdtime >= $allowed_time) {                 
            }
            else 
            {
            ...
            if (isset($json_array['body']['current_password']) && $json_array['body']['current_password'] != "" && isset($json_array['body']['password']) && $json_array['body']['password'] != "") {
                $password_update = 1;
                if (do_hash($user->salt . $json_array['body']['current_password']) != $user->password_hash) 
                {
                    $data['salt'] = $salt;
    ...
    function profile() {        
        ...
        if ($json_array['body']['userId'] != "") 
        {
            ...
            // Validate session code end
            //$user = $this->user_model->find($json_array['body']['userId']);
            $selects = 'id, email, nickname, phone, username';
            $user = $this->user_model->select($selects)->find_by('id', $json_array['body']['userId']);
            if (isset($user) && isset($user->deleted) && $user->deleted >= 1) { 
                echo json_encode(array('status' => 'error', 'message' => 'User account deleted'));
                exit;
    ...
    { 
        ...
        {    
            ...
            if (isset($user) && isset($user->id) && $user->id != '')
            {
                ...
                echo json_encode(array('status' => 'success', 'message' => "Order successfully created", 'order_id'=>$orderid,'pickuptime'=>$data['pickuptime']));
                exit; 
            }
            else 
            {
    ...
    function delete_favourite() {
        ...
        if ($json_array['body']['userId'] != "") 
        {
            ...
            if (isset($user) && isset($user->id) && $user->id != '')
            {
                if ($this->order_favourite_model->delete_where(array("userid"=>$json_array['body']['userId'],"orderid"=>$json_array['body']['orderId'])))
                {
    ...
    function logout() {
        ...
        if ($json_array['body']['userId'] != "") 
        {
            ...
            $this->load->model('users/users_app_login_model');
            $this->users_app_login_model->delete_where(array("userid"=>$json_array['body']['userId']));
